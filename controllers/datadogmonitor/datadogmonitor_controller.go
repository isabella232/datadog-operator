...
const defaultRequeuePeriod = 60 * time.Second
...
func (r *Reconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if r.datadogClient == nil {
		...
		if err != nil {
			...
			return r.updateStatusIfNeeded(logger, instance, newStatus, result)
		}
	}
	...
	if instance.Status.ID == 0 {
		...
		if string(instance.Spec.Type) != string(datadogapiclientv1.MONITORTYPE_METRIC_ALERT) {
			logger.Info("For the alpha version, only metric alert type monitors are supported")
		...
		if err != nil {
			logger.Error(err, "Error when creating monitor")
		}
		...
		if instanceSpecHash != statusSpecHash {
			...
			if err != nil {
				logger.Error(err, "Error when updating monitor")
			}
			newStatus.CurrentHash = instanceSpecHash
		} else {
			...
			if err != nil {
				logger.Error(err, "Error when getting monitor")
			}
		}
	}
	...
	// Set generic conditions in the status
	setNewConditions(newStatus, now, err)
...
func (r *Reconciler) updateStatusIfNeeded(logger logr.Logger, datadogMonitor *datadoghqv1alpha1.DatadogMonitor, status *datadoghqv1alpha1.DatadogMonitorStatus, result ctrl.Result) (ctrl.Result, error) {
	if !apiequality.Semantic.DeepEqual(&datadogMonitor.Status, status) {
		...
		if err := r.Client.Status().Update(context.TODO(), datadogMonitorCopy); err != nil {
			if apierrors.IsConflict(err) {
				logger.V(1).Info("unable to update DatadogMonitor status due to update conflict")
...
func convertStateToStatus(monitor datadogapiclientv1.Monitor, newStatus *datadoghqv1alpha1.DatadogMonitorStatus) {
		...
		if exists {
			...
			for group, monitorStateGroup := range *monitorGroups {
				...
				if isTriggered(string(groupStatus)) {
					triggeredState = append(triggeredState, datadoghqv1alpha1.DatadogMonitorTriggeredState{
						MonitorGroup:     group,
	...
	newStatus.TriggeredState = triggeredState
...
func setNewConditions(status *datadoghqv1alpha1.DatadogMonitorStatus, now metav1.Time, err error) {
	if err != nil {
